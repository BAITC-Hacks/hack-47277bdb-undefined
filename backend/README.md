...
`GET /api/users/me` JWT иесінің профилін қайтарады. `PATCH /api/users/me` тек `firstName` (1–100 таңба), `lastName` (100-ге дейін), `phone` (бос болмаса 5–30 таңба) өрістерін қабылдайды. Берілмеген өрістер сақталады; `lastName`/`phone` үшін `null` не бос жол мәнді тазартады. Email тек оқуға арналған. `role`, `password`, `isActive`, `id` және басқа белгісіз өрістер `422` қатесімен қабылданбайды; жауапта пароль хэші болмайды.
