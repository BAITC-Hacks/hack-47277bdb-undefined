...
router.post(
  ...
  [
    ...
    body('company').optional({ nullable: true, checkFalsy: true }).isString().trim().isLength({ max: 200 }),
    body('message').optional({ nullable: true, checkFalsy: true }).isString().trim().isLength({ max: 3000 }),
  ],
