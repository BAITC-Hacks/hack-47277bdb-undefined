...
export function MainLayout() {
  ...
  const selectCity = async (slug: string) => { setChangingCity(true); setNotice(""); try { await setCity(slug); } catch (reason) { setNotice(extractApiError(reason)); } finally { setChangingCity(false); } };
  const contextErrors = [...new Set([cityContext.error, cartContext.error, saved.error, auth.error].filter(Boolean))];
  return <><header className="site-header"><div className="top-strip"><div className="shell top-inner"><label className="city-picker"><MapPin size={14}/><span className="sr-only">Қала</span><select aria-label="Қала" value={city} disabled={cityContext.loading || changingCity} onChange={(event) => void selectCity(event.target.value)}>{!cities.length && <option value={city}>{cityContext.loading ? "Жүктелуде…" : city}</option>}{cities.map((item) => <option key={item.id} value={item.slug}>{item.name}</option>)}</select></label><div className="top-links"><Link to="/delivery-and-payment">Жеткізу және төлем</Link><Link to="/contacts">Дүкендер</Link><Link to="/faq">Көмек</Link><button className="language" aria-label="Тілді ауыстыру" onClick={() => setLanguage(language === "kk" ? "ru" : "kk")}>{language === "kk" ? "ҚАЗ" : "РУС"}</button></div></div></div>
    <div className="shell main-header"><Link className="logo" to="/">EKT<span>.KZ</span></Link><button className="catalog-trigger" onClick={() => setOpen(!open)} aria-expanded={open}><Menu size={19}/>Каталог</button><form className="search-form" onSubmit={submit}><Search size={20}/><input aria-label="Тауар іздеу" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Тауарды, брендті немесе артикулды іздеу"/><button type="submit">Іздеу</button></form>
      <nav className="header-actions" aria-label="Пайдаланушы әрекеттері"><Link to="/compare" aria-label="Салыстыру"><BarChart3/><small>{saved.comparison.length}</small></Link><Link to="/favorites" aria-label="Таңдаулылар"><Heart/><small>{saved.favorites.length}</small></Link><Link to={auth.user ? "/account" : "/login"} aria-label="Жеке кабинет"><UserRound/></Link><Link className="cart-link" to="/cart"><ShoppingCart/><span><b>{cart?.totalItemCount ?? 0} тауар</b><small>{formatPrice(cart?.subtotal ?? 0)}</small></span></Link></nav>
    </div>
